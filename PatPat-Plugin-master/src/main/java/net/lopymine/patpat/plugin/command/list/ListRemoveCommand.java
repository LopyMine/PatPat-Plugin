...
public class ListRemoveCommand implements ICommand {
    ...
    public List<String> getSuggestions(CommandSender sender, String[] strings) {
        ...
        return PlayerListConfig.getInstance().getNicknameByUuid().entrySet().stream()
                .map(entry -> Objects.equals(entry.getValue(), "?") ? entry.getKey().toString() : entry.getValue())
    ...
    public void execute(CommandSender sender, String[] strings) {
        ...
        try {
            ...
            removeByUuid(sender, uuid);
            return;
        } catch (IllegalArgumentException ignored) {
        }

        removeByName(sender, value);
    }

    private void removeByUuid(CommandSender sender, UUID uuid) {
        OfflinePlayer offlinePlayer = Bukkit.getOfflinePlayer(uuid);
        String nickname = offlinePlayer.getName();
        if (nickname == null) {
            nickname = PlayerListConfig.getInstance().getNicknameByUuid().getOrDefault(uuid, "?");
        }
    ...
    private void removeByName(CommandSender sender, String nickname) {
        PlayerListConfig config = PlayerListConfig.getInstance();

        TextComponent nicknameComponent = Component.text(nickname).color(NamedTextColor.GOLD);
